...
export function frameToSpec(rows: Frame): {  SpecRow[]; missing: string[] } {
  if (!rows.length) return {  [], missing: ["(файл пуст)"] };
...
export function frameToKer(rows: Frame): {  KerRow[]; missing: string[] } {
  if (!rows.length) return {  [], missing: ["(файл пуст)"] };
  const m = colMap(rows[0]);
  const need: Array<[string, string]> = [
    ["идкер", "ИД_КЕР"],
...
export function frameToTmc(rows: Frame): {  TmcRow[]; missing: string[] } {
  if (!rows.length) return {  [], missing: ["(файл пуст)"] };
  const m = colMap(rows[0]);
  const need: Array<[string, string]> = [
    ["идтмцфск", "ИД ТМЦ фск"],
