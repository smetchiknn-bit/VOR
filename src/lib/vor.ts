...
export function getKerId(name: string): { id: number | null; note: string } {
  ...
  if (n.includes("воздуховод")) {
    if (RECT_RE.test(n)) return { id: 1426, note: "Прямоугольное сечение" };
    if (ROUND_RE.test(n)) return { id: 1434, note: "Круглое сечение" };
...
export function findTmc(name: string, article: string, index: TmcIndexEntry[]): TmcMatch {
  ...
  if (!best) return { id: null, score: null, status: "none", note: `Нет ТМЦ, содержащих ключ «${key}»` };
...
export function filterKer(rows: Row[], l2: string, l3: string) {
  ...
  for (const r of rows) {
    ...
    if (r2 === l2n && r3 === l3n) {
      filtered += 1;
      if (!map.has(id))
        map.set(id, { name: String(pick(r, "наименованиекер") ?? ""), unit: String(pick(r, "едизмкер") ?? "") });
    }
  }
...
export async function processVor(args: ProcessArgs): Promise<VorResult> {
  ...
  let kerMap = kerF.map;
  if (kerF.filtered === 0) {
    warnings.push(`Фильтр «Л2 = ${l2}, Л3 = ${l3}» вернул 0 строк базы КЕР — подбор выполнен по всей базе.`);
    const all = new Map<number, { name: string; unit: string }>();
    for (const r of ker.rows) {
      const id = Number(pick(r, "идкер"));
      if (Number.isFinite(id) && !all.has(id))
        all.set(id, { name: String(pick(r, "наименованиекер") ?? ""), unit: String(pick(r, "едизмкер") ?? "") });
    }
    kerMap = all;
  }
  ...
  for (let i = 0; i < n; i++) {
    if (i % 40 === 0) {
      onProgress?.(0.25 + 0.65 * (i / Math.max(n, 1)), `Подбор КЕР и ТМЦ — строка ${i + 1} из ${n}`);
      await tick();
    }
    const r = spec.rows[i];
    ...
    if (ker.id !== null && kerMap.has(ker.id)) {
      ...
      miss = true;
      notFound.push({ system, line, name, qty, what: "Код КЕР", reason: `ИД ${ker.id} отсутствует в отфильтрованной базе КЕР` });
  ...
  return {
    ...
    stats: {
      ...
      fasonCount,
      taCounts,
      kerCodeFreq: [...kerFreq.entries()].map(([code, count]) => ({ code, count })).sort((a, b) => b.count - a.count || a.code - b.code),
      tmcExact,
      tmcGroup,
      tmcAnalog,
    },
