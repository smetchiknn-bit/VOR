...
import { loadExcel, vorBlob, csvBlob, type LoadedFile } from "./lib/excelIo";
import { hasCol, processVor, type VorResult } from "./lib/vor";
...
/* ---------- ErrorBoundary: вместо белого экрана — диагностика ---------- */
        ...
        <div className="blueprint-paper flex min-h-screen items-center justify-center p-6">
          <div className="w-full max-w-xl border-2 border-rust-500 bg-white p-6 shadow-xl">
            ...
            <p className="mt-3 text-[13px] leading-relaxed text-ink-400">
              Приложение перехватило ошибку — это не «белый экран», а диагностическая
...
function AppInner() {
  ...
  const run = async () => {
    ...
    try {
      ...
      // файлы готовы сразу после расчёта: скачивание — прямые ссылки,
      // дублируются в сайдбаре рядом с кнопкой запуска
