    ...
    if not (spec_f and ker_f and tmc_f):
        ...
        try:
            ...
            for i in range(n):
                ...
                npp = "%03d" % system_num[sys_name]  # текст: ведущие нули сохраняются
            ...
            df_vor["№ п/п"] = df_vor["№ п/п"].astype(str)  # «001» не превратится в 1
            ...
            if df_nf.empty:
                df_nf = pd.DataFrame([["—", "—", "Все позиции обработаны", "", "—", "—"]],
                                     columns=["Система", "Строка", "Наименование",
                                              "Кол-во", "Что не найдено", "Причина"])
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                ...
                # формат 0.00 → в русской локали Excel отображается «71,52»
                for col in (7, 10):
            ...
            st.success("ВОР сформирован: **%d** строк (Спецификация: %d · КЕР: %d · ТМЦ: %d · "
                       "заголовки: %d). Код КЕР подобран для %d позиций, Код ТМЦ — для %d, "
                       "с пометкой «Не найдено» — %d." %
